...
class Payment:
    ...
    def add(self, bill_id, amount, payment_type,payment_details):
        sql = "insert into payment (tx_id, amount, payment_type, payment_details, payment_date) \
                values (?,?,?,?,?)"
        response = self.connection.cursor().execute(sql,[bill_id, amount, payment_type,payment_details,self.dt])
    ...
    def get(self,id):
        ...
        result = self.connection.cursor().execute(sql,[','.join([str(i) for i in id])])
